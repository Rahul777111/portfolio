...
// ── Pendulum physics ──────────────────────────────────────────────
// btn  = the toggle button (pivot point)
// cat  = hangs below the button as a secondary pendulum
const btnSwing  = { angle: 0, vel: 0 };
const catSwing  = { angle: 0, vel: 0 };
let lastMX = 0, lastMY = 0, lastT = performance.now();
const BTN_STIFF = 0.038, BTN_DAMP = 0.968;
const CAT_STIFF = 0.028, CAT_DAMP = 0.972;

function addForce(mx, speed, dx) {
  const rect = hangerWrap.getBoundingClientRect();
  const localX = (mx - rect.left) / rect.width - 0.5;
  const force = Math.max(-3.5, Math.min(3.5, localX * speed * 7 + dx * 0.035));
  btnSwing.vel += force * 0.55;
  catSwing.vel += force * 1.1; // cat reacts harder
}

if (hangerWrap) {
  hangerWrap.addEventListener('mouseenter', (e) => { lastMX = e.clientX; lastT = performance.now(); });
...
function animateHanger() {
  // btn pendulum
  btnSwing.vel += -btnSwing.angle * BTN_STIFF;
  btnSwing.vel *= BTN_DAMP;
  btnSwing.angle += btnSwing.vel;
  const ba = Math.max(-22, Math.min(22, btnSwing.angle));

  // cat secondary pendulum (offset from btn angle)
  catSwing.vel += -(catSwing.angle - btnSwing.angle * 0.6) * CAT_STIFF;
  catSwing.vel *= CAT_DAMP;
  catSwing.angle += catSwing.vel;
  const ca = Math.max(-34, Math.min(34, catSwing.angle));

  if (themeBtn) themeBtn.style.transform = `rotate(${ba}deg)`;
  if (cartoon)  cartoon.style.transform  = `translate(-50%, 6px) rotate(${ca}deg)`;
...
function blink() {
  ...
  setTimeout(() => eyes.forEach(e => e.style.transform = 'scaleY(1)'), 120);
...
let tailDir = 1, tailAng = 18;
function wagTail() {
  tailAng += tailDir * 1.4;
  if (tailAng > 30 || tailAng < 6) tailDir *= -1;
  if (tail) tail.style.transform = `rotate(${tailAng}deg)`;
...
function drawParticles() {
  ...
  for(let i=0;i<particles.length;i++){
    for(let j=i+1;j<particles.length;j++){
      ...
      if(dist<100){ pctx.beginPath(); pctx.strokeStyle=`rgba(124,58,237,${0.08*(1-dist/100)})`; pctx.lineWidth=0.5; pctx.moveTo(ax,ay); pctx.lineTo(bx,by); pctx.stroke(); }
    }
  }
